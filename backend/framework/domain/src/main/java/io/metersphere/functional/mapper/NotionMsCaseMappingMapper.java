...
public interface NotionMsCaseMappingMapper {
    ...
    @Select("SELECT notion_page_id, ms_case_id, project_id, notion_db_id, notion_last_edited, ms_last_updated " +
    ...
    @Insert("INSERT INTO notion_ms_case_mapping " +
            "(notion_page_id, ms_case_id, project_id, notion_db_id, notion_last_edited, ms_last_updated) " +
            "VALUES(#{notionPageId}, #{msCaseId}, #{projectId}, #{notionDbId}, #{notionLastEdited}, #{msLastUpdated}) " +
            "ON DUPLICATE KEY UPDATE " +
            "notion_db_id = VALUES(notion_db_id), notion_last_edited = VALUES(notion_last_edited), ms_last_updated = VALUES(ms_last_updated)")
    int insert(NotionMsCaseMapping mapping);

    @Update("UPDATE notion_ms_case_mapping " +
            "SET notion_last_edited = #{notionLastEdited}, ms_last_updated = #{msLastUpdated} " +
